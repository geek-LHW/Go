...
// no argu- ments, and converts each number into units like temperature in Celsius and
...
import (
	...
	"github.com/geek-LHW/Go/ch2/Exercise-2-2/conv"
)
...
func tempString(temp float64) string {
	return fmt.Sprintf("%s = %s = %s", conv.Celsius(temp), conv.CToK(conv.Celsius(temp)), conv.CToF(conv.Celsius(temp)))
}

func lengthString(length float64) string {
	return fmt.Sprintf("%s = %s", conv.Meter(length), conv.MToF(conv.Meter(length)))
}

func weightString(weight float64) string {
	return fmt.Sprintf("%s = %s", conv.Kilogram(weight), conv.KToP(conv.Kilogram(weight)))
}
